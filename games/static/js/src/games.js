...
function GamesXBlock(runtime, element) {
    ...
    function startMatching(firstPage) {
        ...
        //the css style changes made by jquery are not persistent when the background-block element is removed, so the following line does not need to be undone when closeGame is called
    ...
    function formatTime(timeSeconds) {
        ...
        //returns in (HH:M)M:SS format, where (HH:M) only appear if they are relevant
    ...
    function selectContainer(selected) {
        ...
        //false match
        if (!selected.match) {
            //reset the previously selected element's border before the animation
            ...
            var div = document.getElementById((selected.id).substring(1));
            var prev_div = document.getElementById((selected.prev_id).substring(1));

            //make sure event listeners are being removed
            div.addEventListener("animationend", function(){$(selected.id).css({"animation-name": "initial"})});
            prev_div.addEventListener("animationend", function(){$(selected.prev_id).css({"animation-name": "initial"})});
            return;
        }

        //match (hide correct pairs with another class)
        ...
        if (selected.match_count%5 == 0 && selected.matches_remaining > 0) {
            ...
            //this will only have an effect if the progress indicator was created during the start function, no need to check anything
        ...
        //end game after last match is made successfully
    ...
    function endGameMatching(lastPage) {
        //page changes regardless of record
        ...
        if (lastPage.new_record) {
            ...
            var newTime = "<div class='matching-end-record-time'>" + formatTime(lastPage.new_time) + "</div>"; //need to convert new_time to h:m:s format (maybe make a function for this and update timer function)
